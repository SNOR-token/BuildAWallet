...
CHAINS={
 "ethereum":{"family":"evm","chain_id":1,"symbol":"ETH","rpc_env":"ETHEREUM_RPC_URL","capabilities":["native_transfer","erc20","contracts","fee_estimation","simulation","confirmation_tracking"]},
...
def public_registry():
 return [{"id":k,**{x:y for x,y in v.items() if x!="rpc_env"},"configured":bool(os.getenv(v["rpc_env"]))} for k,v in CHAINS.items()]
