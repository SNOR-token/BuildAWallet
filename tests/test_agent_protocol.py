...
def test_policy_operator_separation_and_idempotency(monkeypatch):
...
def test_multifamily_wallet_generation():
 with client:
  a=credential("agent","family-agent");h={"Authorization":f"Bearer {a['token']}"}
  sol=client.post("/v1/wallets",headers=h,json={"chain":"solana"});assert sol.status_code==201;assert len(sol.json()["address"])>=32
  btc=client.post("/v1/wallets",headers=h,json={"chain":"bitcoin"});assert btc.status_code==201;assert len(btc.json()["address"])>=26
  ltc=client.post("/v1/wallets",headers=h,json={"chain":"litecoin"});assert ltc.status_code==201;assert len(ltc.json()["address"])>=26
def test_broadcast_switch_and_controls(monkeypatch):
 monkeypatch.setattr(agent_protocol,"value_usd",fake_value)
 with client:
  ...
  tx=client.post(f"/v1/wallets/{wid}/transactions",headers={**ah,"Idempotency-Key":"no-broadcast"},json={"asset":"ETH","amount":0.0001,"destination":"0x000000000000000000000000000000000000dEaD"});assert tx.status_code==202
